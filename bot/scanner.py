...
def strategy_conflicts_with_a(signal, strategy_a_signals: list) -> bool:
    ...
    if signal.strategy_name != "strategy_b_daily_momentum":
        return False
    return any(
...
async def scan_once(signal_queue: asyncio.Queue, config: dict) -> int:
    ...
    strategy_b_enabled = enabled_strategies.get("strategy_b_daily_momentum", {}).get("enabled", False)
    ...
    for index, symbol in enumerate(watchlist):
        ...
        try:
            ...
            if strategy_b_enabled:
                df_15m_b = compute_indicators_b(df_15m_raw.copy(), strategy_b_cfg)
